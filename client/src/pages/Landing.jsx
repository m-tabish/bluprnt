...
function Landing() {
    ...
    const [bluprnts, setBluprnts] = useState(null);
    ...
    return (
        <div className="bluprnt-background relative w-screen pt-24 font-jetMono text-white ">
            <Navbar />

            {/* Hero Section */}
            <section className='relative  min-w-screen lg:w-5/6 md:w-full mx-auto -top-10 snap-start h-screen flex flex-col justify-center  overflow-hidden'>
            ...
            {/* Bluprnt Library */}
            {/* {!loading && bluprnts ? <div className='min-w-screen font-marker items-center justify-center tracking-wider flex  lg:text-5xl'><TypographyH1>Loading</TypographyH1><LoaderCircleIcon /></div> : <BluprntCarousel className="snap-start" bluprnts={bluprnts} />} */}
            {!loading && <BluprntCarousel className="snap-start" bluprnts={bluprnts} />}

            {/* Featured Section */}
            <section className={`snap-start  mt-36   wF-full h-screen flex flex-col justify-center items-center  text-white`}>
...
function BluprntCarousel({ bluprnts, index, className }) {
    ...
    const [sliderRef, slider] = useKeenSlider({
        ...
        slideChanged(s) {
            setCurrentIndex(s.track.details.rel); // rel = relative index
        },
    });
    ...
    return (
        <section className={`w-full h-screen flex flex-col justify-center items-center overflow-hidden px-4 sm:px-6 lg:px-8 ${className}`}>
            ...
            <div className="flex gap-4 justify-center">
                ...
                <p className='my-auto   '>{currentIndex + 1}/{bluprnts.length}</p>
                <button

...
function ProjectInfo({ project }) {
    return (
        <div className="font-jetMono   ">
            ...
            <Card className="p-1 bg-white/70">
                <CardHeader className="m-0 p-0 flex w-full justify-center">
                ...
                <CardContent className=" mx-6 p-0 mb-2 ">
                    <div className="h-52 overflow-hidden  flex flex-col  p-1 ">
                        ...
                        {project.projectDescription.length >= 200 ? (
                            <div className="flex flex-col"><p>{project.projectDescription.slice(0, 200)}...</p><Button variant="link" className="text-sm self-end">
                                Read More
                            </Button></div>
                        ) : (
                            /* even when there's no button, this empty div
                               takes up the same space so the card height stays identical */
                            <div className="flex flex-col">{project.projectDescription} <div className="h-14"></div></div>
                        )}
                    </div>
                </CardContent>
                <CardFooter className="flex bg-white   py-2   items-center justify-center border-t-2 border-primary text-center">
                    <CardTitle className="flex-1 min-h-full pl-14 font-marker ">{project.projectname}</CardTitle>
